...
std::string mime_detect(const std::string &filename) {
  static const std::unordered_map<std::string, std::string> mime_map = {
      ...
      {"woff2", "font/woff2"},  {"stylesheet","text/css"}};
...
/**
 init the dictionary from a File Descriptor (zero-copy on Android)
 */
void *mdict_init_fd(int fd, bool is_mdd) {
    // Call the new constructor with the is_mdd flag
    auto *mydict = new mdict::Mdict(fd, is_mdd);

  // Explicitly set file type for FD-based initialization
  mydict->set_file_type(is_mdd);
...
void mdict_lookup(void *dict, const char *word, char **result) {
    ...
    std::vector<std::string> results = self->lookup(queryWord);
    std::string s = results.empty() ? "" : results[0];
...
extern "C" char* mdict_atomic_lookup(const char* dictPath, const char* key) {
    try {
        ...
        std::vector<std::string> results = dict.lookup(key);
        std::string html = results.empty() ? "" : results[0];
